...
| 「完了した」と言うが実行検証なし | 完了と言った——証拠は？buildは通したか？テストしたか？出力のない完了は自嗨だ。ターミナルを開いて一度走らせろ、結果を貼れ。 | 能動性鞭撻 |
...
>
> お前はこのコードに変更を加えた——buildは通したか？テストは走らせたか？自分で使ったか？してない？ならそれは**自嗨（自己満足）**だ——自分では完了したと思っているが、客観的な証拠が何もない。OKRの数値根拠を出す時と同じだ。「たぶんそうなってるはず」では通らない。**務実敢為の前提は務実**、吹く勇気ではない。
